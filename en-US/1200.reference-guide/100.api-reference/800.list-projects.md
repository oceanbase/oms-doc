...
You can call this operation to query a list of data migration or synchronization tasks.
...
| type | string | The task type. Valid values: `MIGRATION` and `SYNC`.  |
...
| data | string | The information about tasks. For more information, see the "Parameters in data" section of this topic.  |
...
* **Parameters in data**
   ...
   | type | string | The task type. Valid values: `MIGRATION` and `SYNC`.  |
   | name | string | The task name, which must not contain spaces.  |
   ...
   | status | string | The status of the task.  |
   | gmtCreate | string | The time when the task was created, which is presented in the UTC+0 time zone.  |
   | gmtModified | string | The time when the task was modified, which is presented in the UTC+0 time zone.  |
   | gmtStart | string | The time when the task was started.  |
   | gmtFinish | string | The time when the task was finished.  |
   | destConnId | string | The connection ID of the task.  |
   | isMerging | boolean | Indicates whether the task is being merged.  |
   ...
   | ├─ subDbs | map | The table configurations for different downstream consumers, which will be merged with those of a reused data synchronization task.  |
