...
* **Parameters in data**
    ...
    |    ├─ allowDestTableNotEmpty  | string  | The processing strategy to apply when the target table has records. Valid values: <UI-TERM key="oms-community.New.components.ParameterForm.Ignore">`true`</UI-TERM> and <UI-TERM key="oms-community.New.components.ParameterForm.StopProject">`false`</UI-TERM>. <ul><li>The value `true` specifies to ignore existing data in the target table. If the target is a MySQL database or OceanBase Database Community Edition, OMS Community Edition overwrites the existing data in the target table.  <main id="notice" type='notice'><h4>Notice</h4><p>If you set the value to <b>ture</b>, data is pulled in IN mode for full verification. In this case, the scenario where the target table contains more data than the source table cannot be verified, and the verification efficiency will be decreased. </p></main></li><li>The value `false` specifies to stop migration. If the target table has records, an error is returned during full migration. In this case, clear the existing data in the target table and try again.  <main id="notice" type='notice'><h4>Notice</h4><p>After an error is returned, if you click **Resume** in the dialog box, OMS Community Edition ignores this error and continues to migrate data. Proceed with caution. </p></main></li></ul> |
