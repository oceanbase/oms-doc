...
| ├─ incrSyncCheckpoint | integer | The timestamp for forward incremental synchronization. When the source is a DB2 LUW, OceanBase, MySQL, or Oracle database, the value is a Unix timestamp in seconds.  |
...
| ├─ reverseIncrTransferCheckpoint | integer | The reverse increment timestamp. The value is a Unix timestamp in seconds.  |
