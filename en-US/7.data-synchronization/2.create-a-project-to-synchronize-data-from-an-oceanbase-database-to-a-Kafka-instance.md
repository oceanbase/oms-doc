...
5. Click **Next**. On the **Select Synchronization Objects** page, select a synchronization scope.
   ...
   3. In the **Map Object to Topic** dialog box, select a mapping method.
      ...
      | Select Topic | OMS Community Edition allows you to query Kafka topics. You can click **Select Topic**, and then find and select the topics to be synchronized from the **Select Topic** drop-down list.  You can also enter the name of an existing topic and select it after it appears.  |
...
7. Click **Precheck**.
   ...
   During the **precheck**, OMS Community Edition detects the connection with the destination Kafka instance. If an error is returned:
