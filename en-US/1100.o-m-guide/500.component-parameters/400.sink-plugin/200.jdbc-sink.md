...
* Default value: false.
   ...
   If an error is reported because a field of the source table does not exist in the destination table during full data migration or incremental synchronization between a MySQL database and OMS Community Edition, you can set the `ignoreRedunantColumnsReplicate` parameter to `true` and then resume the data migration task.
...
For example, `datasource.variables._FORCE_PARALLEL_DDL_DOP` specifies the degree of parallelism (DOP). You can set a DOP value as needed. For more information about `_FORCE_PARALLEL_DDL_DOP`, see [Classification and optimization of parallel execution](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000819352).
