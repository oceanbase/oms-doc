...
6. The information about the wait record and ready batch in the DISPATCHER message queue.
...
1. Obtain the component ID of Incr-Sync or Full-Import.
   ...
   5. In the **<UI-TERM key="oms-community.Migration.constants.ViewComponentMonitoring">View Component Monitoring</UI-TERM>** dialog box, view the **<UI-TERM key="oms-community.Detail.components.SyncMonitorDrawer.ComponentId">Component ID</UI-TERM>** of **<UI-TERM>Incr-Sync Incremental Synchronization Component</UI-TERM>** or **<UI-TERM>Full-Import Full Import Component</UI-TERM>**.
...
## Information required for delay

* Screenshots of the task delay, Store delay, and Incr-Sync delay.

  ![latency-zh](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/latency-zh.png)

  As shown in the preceding figure, a data migration or data synchronization task has multiple delay concepts.

  | No. | Delay type | Description |
  |------|----------------|-------------------|
  | 1    |    Task delay     | <ul><li>The Incr-Sync delay is used as the task delay for a data migration or data synchronization task. If multiple Incr-Sync delays exist, the maximum Incr-Sync delay is used. </li><li>The task delay and component delay are calculated based on different scheduling storage, so they may differ. For example, a large number of tasks or a long scheduling time may cause the task delay to be greater than the component delay. This only indicates the difference in delay time.</li></ul>         |
  | 2    |    Store delay      | The time difference between the current time and the time when the Store component fetches change records is the Store delay. This time is calculated based on polling, which is typically 10~30 seconds.  |
  | 3    |    Incr-Sync delay      | The time difference between the current time and the minimum change time of the record written to the destination is the Incr-Sync delay. This time is calculated based on polling. The polling time is specified in the time in the parentheses.      |
