...
* Default value: true. Specify the value based on the types of the source and target.
...
* Default value: true if the source is an Oracle-compatible tenant of OceanBase Database or an Oracle database, and false if otherwise.
...
* Valid values: `true` and `false`.
    ...
    If a task is created not for data restore, OMS Community Edition checks whether the target table is empty when the task is started for the first time. If you set the parameter to `true`, an error is reported if the target table is empty. If you set the parameter to `false`, no error is reported if the target table is empty.
...
## [Hive] Coordinator.json

Supported versions: OMS Community Edition V4.2.8 and later.

* Hive external table mode
...
## [Kafka] shuffleHeartbeatIntervalMs

* Function: This parameter controls the interval of sending heartbeats to prevent excessive heartbeats from affecting downstream consumption. If the value of this parameter is smaller than the interval of sending two heartbeats, the heartbeats sent within this interval will be discarded.

Default value: 0 (indicating that all heartbeats will be sent to Kafka), in milliseconds

* Value selection range: int

* Supported versions: OMS Community Edition V4.2.9-CE and later versions
