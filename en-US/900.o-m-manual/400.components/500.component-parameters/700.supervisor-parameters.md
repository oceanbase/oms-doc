|slug|parameters-of-supervisor-component|
|---|---|
...
| **Parameter** | **Default value** | **<UI-TERM key="oms-community.User.Detail.Description">Description</UI-TERM>** |
| --- | --- | --- |
...
| `metricServerAddr`(metrics.url) | None | The address of the target to which metrics are reported.  |
| `metricServerUsername`(metrics.user) | None | The username of the target to which metrics are reported.  |
| `metricServerPassword`(metrics.password) | None | The password of the target to which metrics are reported.  |
