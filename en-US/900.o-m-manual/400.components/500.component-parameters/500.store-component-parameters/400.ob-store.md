|slug|parameters-of-an-oceanbase-store|
|---|---|
...
| tb_black_list | No | \|                                                                                                                                   | The blocklist of the tables that will not be synchronized.  |
| tb_white_list | Yes | \*.\*.\* | The allowlist of the tables to be synchronized, in the format of tenant name.database name.table name, such as `mytenant1.db1.tb1\|mytenant2.db2.*`. |
| ssl_client_authentication    | Yes    | 0              | Whether to enable secure communication. Valid values: 0 (disabled) and 1 (enabled).|
| ssl_external_kms_info        | Yes    | file           | The secure communication method. Only `file` is supported.|
