|slug|parameters-of-a-mysql-store|
|---|---|
...
| filter.blacklist | Yes | "" | The blocklist, where wildcards are separated with vertical lines (|). Only the `*.*`, `dbName.*`, or `dbName.tableName` format is supported.  |
| filter.conditions | No | None | The allowlist, where wildcards are separated with vertical lines (|). Only the `*.*`, `dbName.*`, or `dbName.tableName` format is supported.  |
| filter.ddl | Yes | "" | The supported DDL statements.  |
| filter.ddlblacklist | Yes | "" | The DDL blocklist.  |
| filter.ddlwhitelist | Yes | "" | The DDL allowlist.  |
