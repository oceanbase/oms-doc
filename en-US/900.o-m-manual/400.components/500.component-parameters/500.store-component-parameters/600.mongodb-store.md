...
| mongodb.whitelist | Yes | N/A | The whitelist. The wildcard expressions are separated by a pipe (|). The supported wildcard expressions are `*.*`, `dbName.*`, and `dbName.tableName`. |
| mongodb.blacklist | No | N/A | The blacklist. The wildcard expressions are separated by a pipe (|). The supported wildcard expressions are `*.*`, `dbName.*`, and `dbName.tableName`. |