...
* Valid values: the values of DbTypeEnum.
...
* Description: specifies additional configuration for the source connection. The format is `map<string,string>`. The following configuration parameters are included:
...
* Description: specifies whether to truncate the target table when there are no unique indexes.
...
* Value type: Character or string.
