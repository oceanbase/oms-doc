# Create a project to migrate data from a TiDB database to a MySQL tenant of OceanBase database
...
* You can migrate only tables with primary keys from a TiDB database to an OceanBase database in MySQL tenant mode.

* You cannot migrate space-containing data from a TiDB database to an OceanBase database in MySQL tenant mode.

* During reverse incremental migration from a TiDB database to an OceanBase database in MySQL tenant mode that is of a version earlier than 3.2x and contains a multi-partition table with a globally unique index, data may be lost if you update the value of the partitioning key of the table.
