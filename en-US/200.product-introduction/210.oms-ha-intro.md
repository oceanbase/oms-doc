...
| **Disaster recovery level** |  | **Deployment requirement** | **<UI-TERM key="oms-community.User.Personal.Description">Description</UI-TERM>** |
| --- | --- | --- | --- |
...
| | **<UI-TERM key="oms-community.components.MonitorDrawer.LogPullComponent">Store</UI-TERM>** | **<UI-TERM>Incr-Sync</UI-TERM>** | **<UI-TERM>Full-Import</UI-TERM>** | **<UI-TERM>Full-Verification</UI-TERM>** |
| --- | --- | --- | --- | --- |
| **Server failure** | ✅ | ✅ | ❌ | ❌ |
| **Component exception** | ✅ | ✅ | ❌ | ❌ |
...
1. OMS Community Edition queries Incr-Sync tasks on the failed server and classifies the Incr-Sync threads into two categories, **<UI-TERM key="oms-community.Operation.Overview.constants.Running">Running</UI-TERM>** and Other, based on their status.
2. OMS Community Edition attempts to terminate Incr-Sync tasks in the **<UI-TERM key="oms-community.Operation.Overview.constants.Running">Running</UI-TERM>** state. Usually, this operation cannot succeed. This is because a failed server cannot receive commands.
...
| **<UI-TERM key="oms-community.Audit.constants.Component">Component</UI-TERM>** | **HA supported?** | **Exception criteria** | **HA behavioral logic** |
| --- | --- | --- | --- |
...
4. If no Store task is in the **<UI-TERM key="oms-community.Detail.components.SyncMonitorDrawer.Running">Running</UI-TERM>** state (works normally and reports heartbeats at the specified interval), the HA module will attempt to create a Store task.
5.  
...
- OMS Community Edition does not support Store sharing between migration tasks. If you have created multiple migration tasks based on the same source database ID, the HA module creates, manages, and maintains a Store task for each task.
- OMS Community Edition supports Store sharing between synchronization tasks. If you have created multiple synchronization tasks based on the same source database ID and enabled Store sharing, the HA module creates and maintains the shared Store task for all those synchronization tasks.
...
The following table describes the system parameters that control the behavior of the HA module. To view and edit the parameters, log in to the OMS Community Edition console, choose **<UI-TERM>System Management</UI-TERM>** > **<UI-TERM>System Parameters</UI-TERM>**, and then search for `ha.config` on the page that appears.

| **Scope** | **Parameter** | **<UI-TERM key="oms-community.Operation.constants.Type">Type</UI-TERM>** | **Default value** | **<UI-TERM key="oms-community.User.New.DescriptionOptional">Description</UI-TERM>**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --- | --- | --- | --- |---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| **Component exception (Store)** | subtopicStoreNumberThreshold | Integer | 5 | The maximum number of Store tasks that can be created for a task-data source pair. When the number of Store tasks reaches the specified value, the HA module does not create a new one.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
