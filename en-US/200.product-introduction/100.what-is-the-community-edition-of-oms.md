|slug|what-is-the-community-edition-of-oms|
|---|---|
...
OMS Community Edition provides a visualized and centralized management platform. You can migrate data with simple configurations in real time. OMS Community Edition aims to help you achieve real-time data migration and synchronization from homogeneous or heterogeneous databases to OceanBase Community Edition at a low cost and low risk.

## Features

OMS supports data migration and synchronization.

* Data migration: Data migration is an one-off task. After the migration is completed, the task resources can be released. This feature is intended for full and incremental data migration between two relational data sources to ensure data consistency between the source and target databases. With this feature, you can migrate inventory data from the source database to the target database and maintain real-time synchronization of incremental data from the source database to the target database.

   data migration tasks are the basic unit of the data migration feature of OMS Community Edition. When you create a migration task, you can specify the migration level, which ranges from table to database. For more information about the task types and migration types supported in data migration, see [Data migration](../600.data-migration/100.data-migration-overview.md).

* Data synchronization: Data synchronization is a continuous operation. After a data synchronization task is created, data will be synchronized continuously to ensure consistency between the source and target databases as well as the real-time flow of data of key business systems. With this feature, you can implement real-time data synchronization between OceanBase Community Edition and Kafka or RocketMQ instances. For more information, see [Data synchronization](../700.data-synchronization/100.data-synchronization-overview.md).

## Benefits

* Support for multiple sources

   OMS Community Edition supports real-time data transmission between OceanBase Community Edition and different types of data sources, such as MySQL, TiDB, Kafka, and RocketMQ, and supports data migration between MySQL-compatible tenants of OceanBase Community Edition. The features that OMS Community Edition provides vary with the types of the source and the target.
...
![architecture1-en](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/architecture1-en.png)
...
OMS Community Edition supports in-service data migration, ensuring service continuity of the source database during the migration and minimizing the impact of data migration on the business. During schema migration, full migration, and incremental data migration, the full data and incremental data in the source database are synchronized to the sink database in real time. After the data verification succeeds, the business can be handed over from the source database to the sink database.
...
### Highly availability and flexible deployment

* Single-node support for high availability (HA) is provided by Store and Incr-Sync components. If a Store or Incr-Sync component fails, the faulty process will be restarted.

  ![architecture2-en](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/architecture2-en.png)

* Disaster recovery support based on IDC.

  ![architecture3-en](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/architecture3-en.png)

  * If the local node fails, the components are migrated to another node.

  * When the resources of the machine are insufficient, OMS Community Edition will attempt to perform load balancing for some components.

  * When the Store fails, the Store will be restarted if there are sufficient resources. If resources are insufficient, OMS Community Edition will perform load balancing or delete redundant stores.

  * The exception process will be restarted when a fault occurs in Incr-Sync.
...
* Support for disaster recovery in region-based data centers.
  ...
  ![architecture4-en](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/architecture4-en.png)
