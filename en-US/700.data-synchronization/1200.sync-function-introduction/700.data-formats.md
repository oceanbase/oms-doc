...
* The `clusterId` field is described as follows:
  ...
  * In an OceanBase database, the `ob_org_cluster_id` parameter specifies the CLUSTER_ID at the session level of the OceanBase database. The value is persisted in the transaction log. If you specify the `ob_org_cluster_id` parameter when you write data, the specified value takes precedence over the default value. The `clusterId` field exists only in incremental synchronization. For more information, see [ob_org_cluster_id](https://www.oceanbase.com/docs/common-oceanbase-database-cn-0000000002183538).
...
```json
{
 "database": "STRING", // The name of the database in which the changed data is located. For OceanBase Database, only the database name is provided without the tenant name.
  ...
  "ts": LONG, // The timestamp when the data is written to the destination data source.
...
```json
{
 "data": { // Key-value pairs of change data. For INSERT and DELETE operations, the key-value pairs are complete values. For UPDATE operations, only the changed values are included.
  ...
  "meta": {
    ...
    "posttime": "YYYY-MM-DDTHH:mm:ss",  // Time when data is written to the destination data source
