...
This topic describes the supported operations on the **User Management** page after you log in to the console of OceanBase Migration Service (OMS) Community Edition as users with different roles.
...
2. In the left-side navigation pane, choose **System Management** > **User Management**.
   ...
   If you log in as the root user or a user with the ROOT-VIEWER role, you can choose **System Management** > **Permission Management**, and filter users by department on the **User Management** page.
...
4. In the **Create User** dialog box, configure the parameters.
   ...
   |------|--------------------------------------------------------------------------|
   | Account | The custom username of the account.                                  |
   ...
   | Department | Only the root user can select a department. When you create a user with the ROOT-VIEWER role, `default` is used by default. To create a department, select **Create Department** from the drop-down list. For more information, see [Manage departments](../100.permission-management/300.department-management.md). |
   | Password | The password must be a string of 8 to 32 characters and contain at least two digits, two upper-case letters, two lower-case letters, and two special characters. The following special characters are supported:<br><code>~ ! @ # % ^ & * _ - + = \ | ( ) { } [ ] : ; , . ? / |</code>.
...
2. In the **Edit User** dialog box, modify the user information as needed.
    ...
    <h4>Notice</h4>
    <p>You cannot change the <strong>account</strong> of any user. </p>
    <p>You cannot change the <strong>account</strong> or <strong>department</strong> of an admin user or a user with the ROOT-VIEWER role. </p>
    </main>
...
## Delete a user
  ...
  <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>You cannot delete the root user or admin users, which are created by the system. </p>
  </main>
