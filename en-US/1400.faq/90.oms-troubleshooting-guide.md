...
The forward switch is performed in the control component. If you select reverse incremental synchronization, the system creates a reverse Store and reverse incremental synchronization components.
...
* The user system is cgroup2, causing the OMS Community Edition machine to be in an abnormal state. OMS V4.2.4 and later versions resolve this issue.
...
Full migration usually requires index creation to improve efficiency. For OceanBase Database as the target database, you can configure the index creation concurrency using the `ob.parallel` parameter in the `struct.transfer.config` system parameter.

### Incremental synchronization

Transaction shard: `source.splitThreshold`. When bulk operations cause incremental delays, you can adjust this parameter. The default value is 128. For `INSERT` operations, a higher value is better, up to a maximum of 512. For `UPDATE` and `DELETE` operations, a lower value is better, with a minimum of 1.

### metrics.log analysis

#### Identify the bottleneck

The data flow is: slice shard -> source reads from the source -> dispatcher distributes data -> sink writes to the target.
...
When you migrate data from multiple tables in a source database to a single table or partitioned table in a destination database, you can use the following example:
...
### OceanBase Database without a primary key - Hidden columns
...
* If you have not performed a forward switch and the destination database will be used as the source database for the next task, an exception will occur. In this case, you need to manually delete the `OMS_PK_INCRMT` column to avoid the exception.
...
If you want to have rollback capabilities or use the original database as a standby database, we recommend that you select **Reverse Incremental Migration** when creating a data migration task. Additionally, if MySQL is used as the source database (for example, MySQL to OceanBase Database), you need to manually create the `omstxndb` transaction database in MySQL. If you do not create this transaction database, an exception will occur with the message `omstxndb` database does not exist.

### Disaster recovery with dual-active
...
## System views used by OMS Community Edition when you select a table or view when you create a task
