# Character set Latin1 encoding issues
...
The Latin1 character set is a byte encoding standard and does not support Chinese. Therefore, when you migrate data of the Latin1 character set, you need to know the character set that the Latin1 character set stores as a byte array. It is usually UTF-8. The process of storing Chinese in the Latin1 character set is as follows:
...
Two new parameters, `latin1byte` and `names`, are added for the source and sink. The supported data sources include MySQL, OceanBase, and TiDB.
...
* `names` specifies whether to execute `set names 'character set'` when you connect to the database.
  ...
  * By default, automatic settings are used in the program. For an OceanBase database, you do not need to set this parameter. For a MySQL database, you can set this parameter to `utf8`.
...
This setting takes effect only when both the source and destination fields are of the Latin1 character set.

* Full migration

    After the source reads data, it stores the data as a string and passes it to the sink. The sink obtains the byte array of the actual Latin1 character set by using `getBytes("actual character set of Latin1")` and then writes the byte array to the destination by using `setBytes`.

* Incremental synchronization

    The incremental synchronization component consumes data from the store and directly uses `setBytes` to write the consumed `bytes` to the destination.
...
Direct load does not support writing data to the Latin1 character set. An exception will be thrown.