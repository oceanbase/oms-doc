...
This topic describes how allowlists and blocklists work and how to update them.

## Applicable versions

OceanBase Migration Service (OMS) 3.2.0 and later

## Working principles of allowlists and blocklists

allowlists and blocklists provide differentiated configuration capabilities, such as database and table filtering, mapping, and extract, transform, and load (ETL). A blocklist or allowlist is in the following structure:
...
A blocklist and a allowlist are two JSON arrays with the same semantics. Each element in an array is a JSON object, and each JSON object contains the following attributes:
...
<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>A match takes effect on the premise that it is in the allowlist but not in the blocklist. </p>
</main>

## Update allowlists and blocklists

You can update a blocklist or allowlist by adding and deleting tables.
...
* Delete a table: Find the table to be deleted and delete it from the allowlist or add the configuration of the table to the blocklist.

The following example updates the blocklist of the Incr-Sync component.

1. Go to the **<UI-TERM key="oms-community.Operator.Store.useStoreAction.UpdateConfigurations">Update Configuration</UI-TERM>** dialog box for the **Incr-Sync** component.
   ...
   3. On the **<UI-TERM key="oms-community.Audit.constants.Component">Component</UI-TERM>** page, click the **Incr-Sync** tab.

   4. On the **Incr-Sync** tab, click **<UI-TERM key="oms-community.Operator.constants.Update">Update</UI-TERM>** in the **<UI-TERM key="oms-community.pages.Endpoint.Operation">Actions</UI-TERM>** column of the target Incr-Sync component.
...
2. In the **<UI-TERM key="oms-community.Operator.Checker.useCheckerAction.UpdateConfigurations">Update Configuration</UI-TERM>** dialog box, hover the pointer over the blank area next to the `sub` parameter under `blackCondition` and click the Add icon that appears.

   ![update-black-1](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/update-black-1.png)

   To update a allowlist, hover the pointer over the blank area next to the `sub` parameter under `whiteCondition` and click the Add icon that appears.
...
9. In the **<UI-TERM key="oms-community.Operator.Store.useStoreAction.UpdateConfigurations">Update Configuration</UI-TERM>** dialog box, add other parameters in the similar way and click **<UI-TERM key="oms-community.Operator.Connector.StartConnectorModal.StartConfirm">OK</UI-TERM>**.
