...
* During reverse incremental migration from a PostgreSQL database to OceanBase Community Edition, if the database contains a multi-partition table that has a globally unique index, data may be lost during migration if you update the value of the partitioning key of the table.
