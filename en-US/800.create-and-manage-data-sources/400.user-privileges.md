...
During schema migration from a PostgreSQL database to a MySQL-compatible tenant of OceanBase Database, you must grant the `SELECT` privilege on tables and views to the migration user. 

During incremental synchronization from a PostgreSQL database to a MySQL-compatible tenant of OceanBase Database, the privileges required for the migration user are as follows:
