|slug|enable-binlogs-for-the-mysql-database|
|---|---|

# Enable binlogs for the MySQL database

When OceanBase Migration Service (OMS) Community Edition is performing full migration tasks, it reads the binlogs of the source MySQL database to synchronize incremental data to the target database in real time. Before migrating incremental data from a MySQL database by using OMS Community Edition, you must enable binlogs for the MySQL database.
...
1. Log in to the host where the self-managed MySQL database resides.
...
4. Log in to the self-managed MySQL database and execute the following SQL statement to confirm that the binlog feature is enabled.
