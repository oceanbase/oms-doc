...
This topic describes how to deploy OceanBase Migration Service (OMS) on a single node.
...
3. Prepare the configuration file.
   ...
   * The following example describes a template of the `config.yaml` file:
     ...
     | cm_url              | The URL of the OMS CM service. Example: http://10.10.10.1:8088.  <br>**Note:** <br> In single-node deployment mode, the IP address of the server where OMS is deployed is used. We recommend that you do not set it to `http://127.0.0.1:8088`.                                              | Yes                          |
