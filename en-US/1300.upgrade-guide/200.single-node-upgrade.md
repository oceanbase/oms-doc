...
This topic describes how to upgrade OceanBase Migration Service (OMS) Community Edition in single-node deployment mode to V4.2.10.
...
1. Back up the database.
   ...
   1. Suspend the container of OMS Community Edition V4.2.9 and record the time as T1.
   ...
   3. Run the following commands to back up the `rm`, `cm`, and `cm_hb` databases as SQL files and make sure that the sizes of the three files are not 0:
      ...
      | rm_429, cm_429, and cm_hb_429 | Specifies to back up the `rm`, `cm`, and `cm_hb` databases as SQL files named in the format of `database name > SQL file storage path.sql`. You need to specify the values based on the actual environment. |
...
3. Start a new container for OMS Community Edition V4.2.10.
...
7. (Optional) To roll back OMS Community Edition, perform the following steps:
    ...
    4. Restart the old container of OMS Community Edition V4.2.9.
