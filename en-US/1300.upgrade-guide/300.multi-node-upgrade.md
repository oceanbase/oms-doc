...
<h4>Notice</h4>
<p>Currently, it is only supported to upgrade from OMS Community Edition V4.2.7 to Community Edition V4.2.8. Upgrading from versions lower than Community Edition V4.2.7 to Community Edition V4.2.8 is not supported.</p>
</main>
...
2. Back up the databases.
   ...
   3. Run the following commands to back up the `rm`, `cm`, and `cm_hb` databases as SQL files and make sure that the sizes of the files are not 0.
      ...
      | rm_427, cm_427, cm_hb_427 | Specifies to back up the `rm`, `cm`, and `cm_hb` databases as SQL files named in the format of `database name > SQL file storage path.sql`. You need to specify the values based on the actual environment.  |
...
4. Start a new container for OMS Community Edition V4.2.8.
   ...
   <p>Before you start the container for OMS Community Edition V4.2.8, make sure that the three disk mounting paths of OMS Community Edition are the same as those before the upgrade. <br>You can run the <code>sudo docker inspect ${CONTAINER_NAME} | grep -A5 'Binds'</code> command to view the disk mounting paths before the upgrade, which are paths to the disks mounted to the old container of OMS Community Edition. </p>
