...
This article introduces how to scale a single region and a single node, as well as a single region with multiple nodes.
...
<p>Execution takes 2 to 4 minutes. Please wait patiently and do not interrupt the execution process. </p>

</main>

   ## Expand in a single region

### Initial configuration

   This example describes how to expand OMS Community Edition V4.2.7-CE in a single region. The initial configuration specifies that the cm_url parameter is set to the value of the VIP, `http://xxx.xxx.xxx.1:1188`.
...
### Procedure
   ...
   In this example, the cm_url parameter is set to the value of the VIP `http://xxx.xxx.xxx.1:1188` based on the initial configuration, and the cm_nodes parameter is set to xxx.xxx.xxx.2 for the new node to be added. The procedure for expanding OMS in a single region is as follows.
