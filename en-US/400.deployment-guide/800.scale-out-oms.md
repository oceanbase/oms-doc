...
This topic describes how to scale out a single region and a single node, as well as multiple nodes in a single region.

## Upgrade from a single-node cluster to a multi-node cluster

You can upgrade a single-node cluster to a multi-node cluster in the following two scenarios:

* The initial cluster configuration specifies a VIP as the cm_url.

* The initial cluster configuration specifies a physical server IP address as the cm_url.

### The cm_url is set to the VIP

The specific steps are the same as those for the initial single-node deployment. For more information, see [Single-node deployment](../400.deployment-guide/500.deploy-oms-on-a-single-node.md).
...
Take the initial node's `cm_nodes` as xxx.xxx.xxx.1 and the scaled-out node's `cm_nodes` as xxx.xxx.xxx.2 as an example. The `config.yaml` file is configured as follows.
...
# When you scale out a single node to multiple nodes, make sure that the names of the following three databases are the same as those on the initial node.
drc_rm_db: ${drc_rm_db}
drc_cm_db: ${drc_cm_db}
...
### The cm_url is set to the IP address of the physical host.
...
## Upgrade a single-region, single-node OMS Community Edition deployment to a multi-region, single-node deployment

The procedure for upgrading a single-region, single-node OMS Community Edition deployment to a multi-region, single-node deployment is the same as the procedure for deploying OMS Community Edition on a single node, except that the `drc_cm_heartbeat_db` parameter in the `config.yaml` file must be different from the heartbeat database used by the initial OMS Community Edition node. You also need to modify the `cm_url`, `cm_location`, `cm_region`, `cm_region_cn`, `cm_is_default`, and `cm_nodes` parameters to the values for the new region.
...
3. Prepare the configuration file.
   ...
   Make sure to replace the required parameters with the actual values for the target environment. Take the example of upgrading the OMS Community Edition node at xxx.xxx.xxx.xx1 in Jiangsu Province. The `config.yaml` file is configured as follows.
   ...
   | oms_meta_password   | The password of the MetaDB.                                                                                                                                                                  | Yes                |
...
7. In the root directory, run the metadata initialization command.
   ...
   4. Initialize resource labels and resource groups of OMS Community Edition.
...
## Multi-node scaling within a single region

### The initial environment is configured with two nodes in a single region, and the CM URL is the VIP.

This topic describes how to scale out the deployment of OMS Community Edition V4.2.7-CE to multiple nodes in a single region by using the `docker_remote_deploy.sh` script.
...
The CM URL specified in the initial node configuration, `http://xxx.xxx.xxx.1:1188`, serves as the VIP, and the CM nodes for the scaled-out nodes is xxx.xxx.xxx.2. This describes the procedure for scaling out a single region to multiple nodes.

1. Configure the VIP, SLB, or domain name and bind them to the newly scaled-in nodes.

2. Copy a copy of the configuration file to the new server. In the `config.yaml` configuration file on the new server, add the IP address xxx.xxx.xxx.2 under the `cm_nodes` parameter.
...
3. Run the command to start the container.
   ...
   |           Parameter           |                                                                                                        Description                                                                                                         |
   |-------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | OMS_HOST_IP             | The IP address of the host. This parameter must be specified as the IP address of the current scale-out server.                                                                                                                                     |
