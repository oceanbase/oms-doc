...
## Expand a single node to multiple nodes

When you expand a single node to multiple nodes, the configuration files differ from those for a single-node deployment. This topic uses the example of expanding a single node to two nodes.

* `cm_nodes` needs to be expanded from one node to two nodes.
...
The steps to expand a single node to multiple nodes are as follows:

1. Enter the k8s environment and prepare the configuration file.

    The following is an example of the configuration file for expanding a single node to multiple nodes. Replace the parameters with their actual values based on your specific situation.
    ...
    template:
        ...
        - name: writable-config
            emptyDir: {}
    # The disk capacity configuration. You can increase the sizes of the following three mounted disks based on your specific situation.
...
## Expand a single region to multiple regions

When you expand a single region to multiple regions, you need to create a configuration file for the new region (you can copy the configuration file of the original region and modify it). The main modifications to the new region's configuration file are as follows:
...
The following example shows how to expand the Hangzhou region to the Hangzhou and Beijing regions.

1. Enter the k8s environment and create a configuration file for the new region, Beijing.

    The following example shows the configuration file for expanding a single region to multiple regions. Please replace the parameters with their actual values as needed.
