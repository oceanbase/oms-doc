...
OMS Community Edition also supports deployment on multiple nodes in a single region. For more information, see [Deploy OMS on multiple nodes in a single region](../400.deployment-guide/500.docker-deploy/200.deploy-oms-on-multiple-nodes-in-a-single-region.md).
...
OMS Community Edition supports multi-region deployment. You can deploy OMS Community Edition across multiple regions. For more information, see [Deploy OMS on multiple nodes in multiple regions](../400.deployment-guide/500.docker-deploy/300.deploy-oms-on-multiple-nodes-in-multiple-regions.md).

## Considerations

* You can directly deploy OMS Community Edition V4.1.1, but cannot upgrade earlier versions to V4.1.1.

* If you need OMS Community Edition to collect and display historical monitoring data, deploy an InfluxDB time-series database. For more information, see [Deploy a time series database](../400.deployment-guide/500.docker-deploy/600.deploy-a-time-series-database.md).

* By default, HA is disabled in OMS Community Edition. If you want to enable HA, log in to the OMS console and change the value of `enable` to `true` in `ha.config`. For more information, see [Modify high availability configurations](../1000.system-management/400.system-parameters/200.modify-ha-configurations.md).
