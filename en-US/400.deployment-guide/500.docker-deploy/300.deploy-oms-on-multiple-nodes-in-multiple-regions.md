|slug|multi-region-multi-node-deployment-docker|
|---|---|
...
* You can deploy OMS Community Edition on a single node first and then scale out to multiple nodes. For more information, see [Scale out OMS Community Edition](../500.docker-deploy/400.scale-out-oms.md).
...
5. Complete the deployment as prompted. After you set each parameter, press **Enter** to move on to the next parameter.
   ...
   3. Enter the configuration information of the MetaDB cluster as follows:
       ...
       3. Determine whether to enable SSL encryption for the metadata database connection.

          If you enter y, SSL will be enabled for the metadata database. Please provide the paths for the CA root certificate file, client certificate file, and client private key file as prompted.
...
* Here is a sample template of the `config.yaml` file for you to deploy OMS Community Edition in the Hangzhou region:
   ...
   meta_ssl_enabled: true
   meta_ssl_ca: ${ca_path}
   meta_ssl_client_cert: ${client_cert_path}
   meta_ssl_client_key: ${client_key_path}
   ```
   ...
   | meta_ssl_enabled | Whether to enable SSL. BOOLEAN. The default value is `false`. | No |
   | meta_ssl_ca | The CA certificate path. STRING. | No |
   | meta_ssl_client_cert | The client certificate path. STRING. | No |
   | meta_ssl_client_key | The client keystore path. STRING. | No |
...
* Here is a sample template of the `config.yaml` file for you to deploy OMS Community Edition in the Heilongjiang region:
   ...
   meta_ssl_enabled: true
   meta_ssl_ca: /obdata/data/xxx/ssl/ob/ca.pem
   meta_ssl_client_cert: /obdata/data/xxx/ssl/ob/client-cert.pem
   meta_ssl_client_key: /obdata/data/xxx/ssl/ob/client-key.pem
   ```
