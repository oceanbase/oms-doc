|slug|synchronize-ddl-operations-between-obmysql|
|---|---|

# Supported DDL operations in incremental migration between MySQL-compatible tenants of OceanBase Database

You can use DDL operations to migrate incremental data between MySQL-compatible tenants of OceanBase Database.

> **Notice**
>
> If you have created an `index` in the source tenant and it also exists in the target tenant, OMS Community Edition automatically ignores the `index` without interrupting the migration process.
...
* `ALTER TABLE MODIFY COLUMN`
   ...
   <main id="notice" type='notice'>
    <h4>Notice</h4>
    <p>You can only extend the column length. You cannot modify the column type.</p>
   </main>
...
## Limitations of DDL operations in incremental migration
