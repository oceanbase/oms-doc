...
1. Create a task to migrate data from HBase to OBKV and configure the task until the **Migration Options** step.
...
4. In the **New Computing Platform** dialog box, configure the following parameters.
    ...
    | Config File | The configuration file of the source HBase cluster, which is named `hbase-site.xml`.  |
