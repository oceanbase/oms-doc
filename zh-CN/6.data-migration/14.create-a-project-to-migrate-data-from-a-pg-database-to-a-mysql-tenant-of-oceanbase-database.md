# 创建 PostgreSQL 至 OceanBase 社区版的数据迁移项目

本文为您介绍如何使用 OceanBase 迁移服务（OceanBase Migration Service，OMS）迁移 PostgreSQL 数据库的数据至 OceanBase 社区版。

## 背景信息

在 OMS 控制台创建从 PostgreSQL 数据库迁移数据至 OceanBase 社区版的数据迁移项目，您可以通过结构迁移、全量迁移和增量同步，无缝迁移源端数据库中的存量业务数据和增量数据至 OceanBase 社区版。
...
* 已在目标端 OceanBase 社区版中创建对应的 Schema。OMS 支持迁移表和列，您需要提前在目标端创建对应的 Schema。
...
* OMS 支持迁移 PostgreSQL 数据库的有主键表、NOT NULL UK 表至 OceanBase 社区版。

* OMS 迁移 PostgreSQL 数据库的数据至 OceanBase 社区版时，不支持同步 DDL。
...
7. 单击 **下一步**，在 **选择迁移对象** 页面，选择迁移对象和迁移范围。
   ...
   * 选择 **指定对象**，在左侧选中需要迁移的对象，单击 **\>**，将其添加至右侧列表中。您可以选择一个或多个库的表、视图作为迁移对象。
     ...
     迁移 PostgreSQL 数据库的数据至 OceanBase 社区版时，OMS 支持通过文本导入对象、重命名对象名称、设置行过滤、查看列信息，以及移除单个或全部迁移对象。
