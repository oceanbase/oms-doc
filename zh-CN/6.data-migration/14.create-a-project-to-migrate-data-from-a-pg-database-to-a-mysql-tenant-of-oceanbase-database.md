...
2. 在 **选择源和目标** 页面，配置各项参数。
   ...
   | 目标端    | 如果您已创建 OceanBase 数据库 MySQL 租户数据源，请从下拉列表中进行选择。如果未创建，请单击下拉列表中的 **新建数据源**，在右侧对话框进行新建。参数详情请参见 [新建 OceanBase 物理数据源](../8.create-and-manage-data-sources/1.create-a-data-source/1.create-an-oceanbase-data-source/1.create-oceanbase-database-physical-tables-as-a-data-source.md)。                                     |
...
5. 在 **选择迁移类型** 页面，配置各项参数。
   ...
   **迁移类型** 包括 **结构迁移**、**全量迁移**、**增量同步**、**全量校验** 和 **反向增量**。
   ...
   | 全量校验 | <ul><li> 如果选择 **全量校验**，建议您在全量校验开始前，分别收集 PostgreSQL 数据库和 OceanBase 数据库 MySQL 租户的统计信息。OceanBase 数据库 MySQL 租户的收集详情请参见 [手动收集统计信息](https://www.oceanbase.com/docs/enterprise/oceanbase-database-cn/V3.2.3/10000000000357540)。   <li>如果您选择了 **增量同步**，且 **同步 DML** 选项中未选择所有的 DML，则 OMS 不支持本场景下的全量数据校验。<li>OMS 仅支持对于具备唯一键的表（主键或唯一约束，且字段具有唯一性）进行全量数据校验。</ul>                                                                                                                                     |
   | 反向增量 | 以下情况均不支持选择 **反向增量**： <ul><li>存在多表汇聚的情况。  <li>存在 Schema 多到一映射的情况。              |

6. （可选）单击 **下一步**。如果您选择了 **反向增量**，且目标端 OceanBase 数据库 MySQL 租户数据源未配置 Configurl、用户名和密码，则会弹出 **数据源补充信息** 对话框，提醒您配置相应参数。参数详情请参见 [新建 OceanBase 物理数据源](../8.create-and-manage-data-sources/1.create-a-data-source/1.create-an-oceanbase-data-source/1.create-oceanbase-database-physical-tables-as-a-data-source.md)。
