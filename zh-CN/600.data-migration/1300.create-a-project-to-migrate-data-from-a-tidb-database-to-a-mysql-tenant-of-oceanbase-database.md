...
TiDB 数据库支持在线事务处理和在线分析处理（Hybrid Transactional and Analytical Processing，HTAP），是一款融合型分布式数据库产品。当您使用 TiDB 数据库 V4.x 及之后版本时，您需要部署 TiCDC 集群和 Kafka 集群来实现 TiDB 数据库至 OceanBase 社区版的增量数据同步。
...
8. 单击 **下一步**，在 **迁移选项** 页面，配置各项参数。
   ...
   * 增量同步
      ...
      | Kafka 消费 group.id（可选） | `group.id` 为 Kafka 中消费者组的唯一标识符。|
