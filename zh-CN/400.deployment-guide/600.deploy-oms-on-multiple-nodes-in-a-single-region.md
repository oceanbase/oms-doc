...
>**注意：**
>
>高可用功能默认是关闭的。如果您需要保障 Store/JDBCWriter/Connector 等组件的高可用能力，请在控制台手动开启。详情请参见 [修改高可用功能的配置](../1000.system-management/400.system-parameters/200.modify-ha-configurations.md)。
...
5. 根据工具提示完成部署，每次输入后，通过回车进入下一步。
   ...
   14. 对每个节点重复 **输入用户名和密码，以及挂载根目录等部署一个节点时需要的额外信息** 步骤，直至完成部署。
