...
| spring_security_oauth2_client_registration_oms_redirect_uri | 即《配置回调地址》步骤中配置的 `redirectUrl`，格式为 `http://{serverDomain}/login/oauth2/code/{registrationId}`。<br>其中 {serverDomain} 为用户域名，{registrationId} 无需填充。  | 必选                |
