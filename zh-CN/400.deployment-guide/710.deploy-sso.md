...
本文以单节点部署为例，为您介绍如何实现单点登录方式登录 OMS 社区版。如果您需要进行单地域多节点或多地域多节点部署，请参见 [单地域多节点部署](../400.deployment-guide/500.docker-deploy/200.deploy-oms-on-multiple-nodes-in-a-single-region.md) 中《部署流程（有配置文件）》模块的内容。
...
2. （可选）部署时序数据库。
   ...
   如果您需要 OMS 社区版能够收集和展示监控数据，请部署时序数据库。如果无需展示监控数据，请跳过本步骤。操作详情请参见 [部署时序数据库](../400.deployment-guide/500.docker-deploy/600.deploy-a-time-series-database.md)。
...
|         参数          |                            描述                     |       是否必选        |
|---------------------|-------------------------------------------|---------------------------------------------------|
| oms_meta_host       | 元信息库的 IP 地址，可以是 MySQL 或 OceanBase 数据库 MySQL 兼容模式。<br> **注意：** <br> OceanBase 数据库需要是 V2.0 及之后版本。        | 必选                |
...
| oms_oauth2_sso_login_url | 接入 OIDC 协议进行单点登录重定向的 URL，格式为 `http://{cm_url}/omsp/oauth2/authorization/oms?oms_back_url={serverDomain}`。其中 `{serverDomain}` 为用户域名，`oms_back_url` 为登录成功后的前端页面。 | 必选                |
...
|  spring_security_oauth2_client_registration_oms_authorization_grant_type | OMS 社区版与 OAuth2 授权服务器交互时使用的授权类型，目前仅支持 `authorization_code` 模式。  | 必选                |
| spring_security_oauth2_client_registration_oms_scope | 权限标识，使用英文逗号（,）分隔，且必须包含 `openid`。  |必选|
|spring_security_oauth2_client_registration_oms_provider | 授权服务提供者名称，您可以自定义。例如，oidc。  | 必选                |
