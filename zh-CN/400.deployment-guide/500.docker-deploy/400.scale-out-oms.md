...
本文为您介绍如何进行单地域和单节点扩容，以及单地域多节点扩容。
...
具体操作步骤，与您最初进行单节点部署时的各步骤保持一致即可。详情请参见 [单节点部署](../500.docker-deploy/100.deploy-oms-on-a-single-node.md)。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>启动容器时，<code>-e OMS_HOST_IP</code> 的值需要配置为当前机器的 IP。</p>
  </main>
...
<h4>说明</h4>
<p>您需要订正的表包括 <code>drc_cm_db</code> 数据库中的表 <code>location_cm</code>、<code>host</code> 和 <code>resource_group</code>，以及 <code>drc_rm_db</code> 数据库中的表 <code>cluster_info</code>。 </p>
</main>
...
6. 复制一份相同的配置文件，在另外一台机器上启动 OMS 社区版容器。
   ...
   # 仅在 OMS 元数据库启用了 SSL 时，需要设置如下三个参数
   -v /data/oms/ssl/ca.pem:/home/admin/conf/meta_ssl/ca.pem 
   -v /data/oms/ssl/client-cert.pem:/home/admin/conf/meta_ssl/client-cert.pem
   -v /data/oms/ssl/client-key.pem:/home/admin/conf/meta_ssl/client-key.pem
...
3. 准备配置文件。
   ...
   请注意将必填参数替换为您部署的目标环境的实际值。以扩容机器 xxx.xxx.xxx.xx1，地域江苏为例，`config.yaml` 文件的配置如下。
...
5. 执行以下命令，启动容器。
   ...
   | IMAGE_TAG               | 使用 Docker 加载 OMS 安装包后，通过 `docker images` 命令获取到加载的相应镜像的 [IMAGE ID] 或 [REPOSITORY:TAG]，即为相应加载后镜像的唯一标识符 `<OMS_IMAGE>`。<main id="notice" type='notice'><h4>注意</h4><p>请替换 <code>work.oceanbase-dev.com/obartifact-store/oms:${IMAGE_TAG}</code> 为执行 <code>docker load -i</code> 命令实际导入的镜像名称。</p></main>                                           |
   | /data/oms/oms_logs <br> /data/oms/oms_store <br>  /data/oms/oms_run  | `/data/oms/oms_logs`、`/data/oms/oms_store` 和 `/data/oms/oms_run` 可以替换为在您的 OMS 社区版部署服务器上创建的挂载目录，分别存放 OMS 社区版运行过程中产生的日志文件、日志拉取组件和同步组件生成的文件，在本机进行持久化。 <br>**注意：**<br>  在后续的版本重新部署、升级中，挂载目录的位置必须保持不变。 |
   | /home/admin/logs <br> /home/ds/store<br>  /home/ds/run       | `/home/admin/logs`、`/home/ds/store` 和 `/home/ds/run` 为容器内的固定目录，路径不可以修改。                                                                                                                                           |
   | /data/oms/ssl/ca.pem（可选） <br> /data/oms/ssl/client-cert.pem（可选） <br> /data/oms/ssl/client-key.pem（可选） | 元数据库的 SSL 证书在 OMS 社区版容器内的挂载位置。如果 OMS 元数据库启用了 SSL，则需要进行配置。这三个文件在容器内的路径不支持修改，仅支持修改宿主机的路径。|
...
7. 在 root 目录下，执行元数据初始化操作。
   ...
   <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>执行过程需要耗时 2~4 分钟，请您耐心等待，切勿中断执行。</p>
   </main>

## 单地域多节点扩容

### 初始环境配置的是单地域两节点，cm_url 为 VIP

本文以 OMS 社区版 V4.2.7-CE 为例，为您介绍单地域多节点的扩容 `docker_remote_deploy.sh`。

1. 拉取 OMS 社区版 V4.2.7-CE 镜像，从加载的镜像中获取部署脚本。
...
2. 通过部署脚本启动部署工具。
   ...
   ```shell
   sudo sh docker_remote_deploy.sh -o <OMS 容器挂载目录> -i <本机 IP 地址> -d <OMS_IMAGE>
   ```

3. 根据工具提示完成单地域两节点的部署。详情请参见 [单地域多节点部署](../500.docker-deploy/200.deploy-oms-on-multiple-nodes-in-a-single-region.md)。

4. 等待初始化步骤执行成功，登录 OMS 社区版控制台进行确认。

### 扩容步骤

以初始节点配置的 cm_url 为 VIP `http://xxx.xxx.xxx.1:1188`，扩容节点的 cm_nodes 为 xxx.xxx.xxx.2 为例，单地域多节点的扩容步骤如下。

1. 配置好 VIP、SLB 或域名，绑定添加新扩容的节点。

2. 复制一份相同的配置文件到扩容机器上，在复制的 `config.yaml` 配置文件中的 `cm_nodes` 参数下添加需要扩容的节点 xxx.xxx.xxx.2。
...
3. 执行启动容器的命令。
   ...
   |           参数            |                                                                                                        描述                                                                                                         |
   |-------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | OMS_HOST_IP             | 宿主机的 IP 地址。该参数的值需要配置为当前扩容机器的 IP。                                                                                                                                     |
   | CONTAINER_NAME          | 创建容器的名称。格式为 oms_xxx，请根据具体版本填写 xxx。                                                       |
   | IMAGE_TAG               | 使用 Docker 加载 OMS 社区版安装包后，通过 `docker images` 命令获取到加载的相应镜像的 [IMAGE ID] 或 [REPOSITORY:TAG]，即为相应加载后镜像的唯一标识符 `<OMS_IMAGE>`。<main id="notice" type='notice'><h4>注意</h4><p>请替换 <code>work.oceanbase-dev.com/obartifact-store/oms:${IMAGE_TAG}</code> 为执行 <code>docker load -i</code> 命令实际导入的镜像名称。</p></main>                                                                                                       |
...
5. 在 `root` 目录，执行元数据初始化操作。

   ```shell
   bash /root/docker_init.sh
   ```

6. 等待初始化步骤执行成功，登录 OMS 社区版控制台进行确认。
