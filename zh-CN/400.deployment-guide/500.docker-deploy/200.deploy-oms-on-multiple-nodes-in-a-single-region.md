...
* 确认安装环境符合系统和网络要求，详情请参见 [系统和网络要求](../200.system-requirements.md)。
...
5. 根据工具提示完成部署，每次输入后，通过回车进入下一步。
   ...
   4. 请输入下述 OMS 社区版的集群配置信息：
      ...
      1. 设置是否自定义组件端口号，并配置相关参数。

         此处输入 y，表示需要进行自定义组件端口号配置，请根据界面提示，依次输入 Nginx、GHANA、CM、Supervisor 和 sshd 服务端口。

      2. 请输入 OMS 社区版集群管理服务的地址，即当前地域所有 CM 机器挂载的同一个 VIP 或域名，原配置项名称为 `cm-url`。
      ...
      3. 请依次输入本地域中所有机器的 IP 地址，使用英文逗号（,）分隔。

      4. 请确认展示的 OMS 社区版集群配置信息。
...
|         参数          |                                                                                                                                                         描述                                                                                                                                                          |       是否必选        |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------|
| oms_meta_host       | 元信息库的 IP 地址，可以是 MySQL 数据库或 OceanBase 数据库 MySQL 兼容模式。                                                                                                                                                                          | 必选                |
...
| cm_url              | OMS 社区版集群管理服务的地址。例如，<http://VIP:8088>。 <br>**说明：** <br> 单地域多节点部署时，`cm_url` 需要配置为当前地域所有 CM 隶属的一个 VIP 或域名下。 <br>不建议您使用 <http://127.0.0.1:8088>，因为该地址无法扩容为多地域多节点模式。 <br>OMS 社区版控制台的访问地址为：`部署 OMS 社区版的宿主机的 IP 地址:8089`。例如，`http（https）://xxx.xxx.xxx.1:8089`。8088 是负责调度的程序端口，8089 是网页地址的端口。 | 必选                |
...
| cm_is_default       | 表示是否默认 OMS 社区版集群管理服务。                                                                                                                                                                                                                                                                                                  | 可选，默认值为 `true`    |
