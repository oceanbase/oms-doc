...
6. 复制一份相同的配置文件，在另外一台机器上启动 OMS 社区版容器。
   ...
   <main id="notice" type='notice'>
    ...
    <li>
    <p>请替换 <code>work.oceanbase-dev.com/obartifact-store/oms</code> 为执行 <code>docker load -i</code> 命令获取的实际名称。</p>
    </li
    </ul>
   </main>
...
5. 执行以下命令，启动容器。
   ...
   | IMAGE_TAG               | 使用 Docker 加载 OMS 安装包后，通过 `docker images` 命令获取到加载的相应镜像的 [IMAGE ID] 或 [REPOSITORY:TAG]，即为相应加载后镜像的唯一标识符 `<OMS_IMAGE>`。<main id="notice" type='notice'><h4>注意</h4><p>请替换 <code>work.oceanbase-dev.com/obartifact-store/oms</code> 为执行 <code>docker load -i</code> 命令获取的实际名称。</p></main>                                           |
