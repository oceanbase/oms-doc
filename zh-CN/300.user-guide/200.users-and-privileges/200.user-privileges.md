...
* MySQL 增量同步时，数据库用户需要具备 `REPLICATION CLIENT` 和 `REPLICATION SLAVE` 权限。
  ...
  如果您在添加 MySQL 数据源时，勾选了 **增量同步时，允许 OMS 向该实例自动写入增量心跳数据**，OMS 社区版会向对应的 MySQL 数据库创建并更新 `drc.heartbeat` 表。此时，MySQL 数据库用户需要具备创建表、写表的权限。添加数据源详情请参见 [添加 MySQL 数据源](../300.manage-data-sources/100.add-a-data-source/200.add-a-mysql-data-source.md)。
...
如果 Kafka 存在鉴权，请参见 [添加 Kafka 数据源](../300.manage-data-sources/100.add-a-data-source/400.add-a-Kafka-data-source.md)。
