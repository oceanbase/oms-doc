...
3. 单击目标参数 `migration.checker.params.*` **取值** 列下的编辑图标。
    ...
    `migration.checker.params.*` 包括 `migration.checker.params.fast`（快速档位）、`migration.checker.params.normal`（正常档位）和 `migration.checker.params.steady`（平稳档位），您需要根据创建数据迁移任务时在 **迁移选项** 页面选择的 **全量迁移并发速度** 的档位来确定需要编辑的参数。此处以 `migration.checker.params.fast` 为例。
