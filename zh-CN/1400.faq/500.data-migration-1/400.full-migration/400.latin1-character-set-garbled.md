...
3. 使用 `getBytes()` 将字符集转换为字节数组。

4. 使用 `new String(bytes,"utf8")` 将字节数组按照 UTF-8 的编码规则进行转换，获取正确的中文数据。
...
* 全量迁移
    ...
    源端读取后会以 `StringFieldValue` 类型存储传递给 Sink 端，Sink 端获取 Latin1 实际字符集字节数组 `getBytes("latin1实际字符集")`，再使用 `setBytes` 写入目标端。
