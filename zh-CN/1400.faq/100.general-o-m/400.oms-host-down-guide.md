...
OMS 社区版控制台的 **运维监控** \> **机器** 页面显示机器 **宕机**，或者数据迁移/数据同步任务执行过程中遇到报错：`no enough host resource for XXX`。
...
1. 登录 OMS 社区版控制台，在左侧导航栏单击 **运维监控** > **机器**，查询机器状态。
   ...
   如果状态显示为 **在线**，请检查机器实际资源使用情况，并通过停止、释放长期未使用的任务，以降低机器资源消耗。如果显示为 **宕机**，则继续往下排查。
