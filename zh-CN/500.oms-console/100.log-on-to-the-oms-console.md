...
1. 在 102 及之后版本的 Chrome 浏览器中，输入 OMS 社区版的访问地址，按回车键。
   ...
   如果您在部署 OMS 社区版时，已接入 OIDC 协议实现单点登录，则登录页面会显示 **第三方登录** 按钮。您可以单击 **第三方登录**，跳转至 SSO 系统的登录页面，无需再进行下文的操作。部署详情请参见 [OMS 社区版接入 OIDC 协议实现单点登录](../400.deployment-guide/710.deploy-sso.md)。

      ![sso-zh](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/sso-zh.png)
...
2. （可选）设置 admin 和 root 初始密码。
   ...
   1. 在对话框中，输入需要设置的 **<UI-TERM key="oms-community.Endpoint.components.SybaseForm.Password">密码</UI-TERM>**。

      密码需要至少包含 2 位数字、2 位大写字母、2 位小写字母和 2 位特殊字符 (~!@#%^&*_-+=|(){}[]:;,.?/)。

   2. 在 **<UI-TERM key="oms-community.components.ResetPassword.ResetOwnPassword.ConfirmPassword">确认密码</UI-TERM>** 文本框中，再次输入密码。
...
4. 单击 **<UI-TERM key="oms-community.Audit.constants.Login">登录</UI-TERM>**。
