...
2. 在 **选择源和目标** 页面，配置各项参数。
   ...
   | 标签（可选）     | 单击文本框，在下拉列表中选择目标标签。您也可以单击 **管理标签**，进行新建、修改和删除。详情请参见 [通过标签管理数据同步任务](1100.manage-a-data-synchronization-projects/300.use-tags-to-manage-data-synchronization-projects.md)。                                   |
...
3. 单击 **下一步**，在 **选择同步类型** 页面，选择当前数据同步任务的同步类型。
...
4. （可选）单击 **下一步**。
   ...
   源端为 OceanBase 社区版时，结构迁移、增量同步需要配置 **所属 OCP**（可选）、**DRC 用户用户名** 和 **密码**。
...
5. 单击 **下一步**，在 **选择同步对象** 页面，选择同步范围。
   ...
   您可以通过 **指定对象** 和 **匹配规则** 两个入口选择同步对象。本文为您介绍通过 **指定对象** 方式选择同步对象的具体操作，配置匹配规则的详情请参见 [配置同步对象的匹配规则](../700.data-synchronization/1200.sync-function-introduction/300.configure-matching-rules-for-sync-objects.md)。
...
6. 单击 **下一步**，在 **同步选项** 页面，配置各项参数。
   ...
   |     参数     |                            描述                             |
   |------------|-------------------------------------------------------------|
   | 增量同步起始位点   | <ul><li> 如果选择同步类型时已选择 **全量同步**，此处默认为任务启动时间，不支持修改。   <li> 如果选择同步类型时未选择 **全量同步**，请在此处指定同步某个时间节点之后的数据，默认为当前系统时间。您可以选择时间节点，也可以直接输入时间戳。<br> **注意：** <br> 仅支持选择当前时间，或当前时间之前的时间点。 该位点与当前归档日志的保留时间密切相关。如果无特殊要求，可以从当前位点开始启动。  </ul>       |
