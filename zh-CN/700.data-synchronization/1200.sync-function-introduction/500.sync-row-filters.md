...
本文为您介绍创建数据同步任务时，选择同步对象后通过 SQL 条件过滤数据的适用场景、使用限制和操作步骤。

## 适用场景

创建数据同步任务时，您可以设置 SQL 过滤条件来过滤数据。完成设置后，只有满足过滤条件的数据才会被同步或同步至目标数据库。该功能适用于数据的定期同步或迁移、拆分数据表、过滤历史数据和脏数据等多种应用场景。
...
3. 单击显示的 **<UI-TERM key="oms-community.biz.ETL.ETLFormDrawer.Set">设置</UI-TERM>**。

    ![row-filter-3-zh](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/row-filter-3-zh.png)

4. 在 **<UI-TERM key="oms-community.biz.ETL.ETLFormDrawer.Set">设置</UI-TERM>** 对话框中，输入标准的 SQL 语句中的 `WHERE` 子句，来配置行过滤。
...
6. 单击 **<UI-TERM key="oms-community.Endpoint.New.Add">确定</UI-TERM>**。
