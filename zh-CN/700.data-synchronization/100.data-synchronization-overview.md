|slug|data-synchronization-overview|
|---|---|
...
从源端至目标端的数据同步任务主要支持增量数据同步场景，即实时写入源端发生的 DML 变更至目标端。OMS 社区版支持同步的粒度最小为表级别，最大为租户级别。

OMS 社区版为您提供同步数据的功能，支持的任务类型如下：
