...
OceanBase 迁移服务（OceanBase Migration Service，OMS）支持 Admin 用户修改系统参数，普通用户仅支持查看系统参数。
...
2. 在左侧导航栏，单击 **系统管理** \> **系统参数**。
   ...
   **系统参数** 页面为您详细列举每个配置项的 **参数名称**、**取值**、**所属模块**、**描述** 和 **修改时间**。
   ...
   |                     参数                     |                                                                                                                                                                                                                                                                                                                                                                        描述                                                                      |                                                                                                                                    默认值                                                                                                                                    |
   |--------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
   |operation_audit_log.enable|  操作审计功能的开关。 | false |
   |operation_audit_log.retention_time| 操作审计记录的保留时间。建议取值范围在 1 至 1095 之间，单位为天。| 7 |
   |oms.captcha.enable |验证码开关，默认关闭。修改为 true 后，登录页面会展示图片验证码，验证码 10 分钟超时失效。此时登录 OMS 需要输入图片验证码的内容，超时或输入错误都会导致登录失败。|false|
   ...
   |mysql_to_obmysql.charset.mapping| MySQL 数据库至 OceanBase 数据库 MySQL 租户的数据迁移项目中，对不支持编码的转换规则。|[] <br>例如，[{"charset":"utf16le","mappedCharset":"utf16"},{"charset":"*","mappedCharset":"utf8mb4"}]|
