...
* MySQL 增量同步时，数据库用户需要具备的权限。
  ...
  如果您在添加 MySQL 数据源时，勾选了 **增量同步时，允许 OMS 向该实例自动写入增量心跳数据**，OMS 会向对应的 MySQL 数据库创建并更新 `drc.heartbeat` 表。此时，MySQL 数据库用户需要具备创建表、写表的权限。添加数据源详情请参见 [新建 MySQL 数据源](../8.create-and-manage-data-sources/1.create-a-data-source/2.create-a-mysql-data-source.md)。
...
* 对 OceanBase 社区版和 MySQL 数据库具备 `SELECT` 权限。
