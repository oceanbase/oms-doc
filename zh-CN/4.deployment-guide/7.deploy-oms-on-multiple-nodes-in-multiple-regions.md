...
* 已在每个节点的机器上，加载下载的 OMS 社区版安装包至 Docker 容器的本地安装包仓库。

   `docker load -i <OMS 社区版安装包>`

   本文以加载的安装包名称为 `OMS_IMAGE` 为例，您在部署时，请根据实际安装包名称进行替换。
...
5. 根据工具提示完成部署，每次输入后，通过回车进入下一步。
   ...
   13. 根据业务需求，修改 `config.yaml` 文件。

   14. 执行命令 `python -m omsflow.scripts.units.oms_init_manager --init-config-file`。

   15. 执行命令 `supervisorctl restart oms_console oms_drc_supervisor`。
