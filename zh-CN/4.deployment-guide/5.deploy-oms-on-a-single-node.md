...
本文为您介绍如何在单节点中使用 OMS 部署工具安装 OceanBase 迁移服务(OceanBase Migrat ion Service，OMS)。
...
| cm_url              | OMS 集群管理服务的地址。例如，`http://10.10.10.1:8088`。 <br>**说明：** <br> 单节点部署时，通常配置为当前 OMS 机器 IP。不建议您使用 `http://127.0.0.1:8088`。 <br>OMS 控制台的访问地址为：`部署 OMS 的宿主机的 IP 地址:8089`。例如，`http（https）://10.10.10.1:8089`。 <br>8088 是负责调度的程序端口，8089 是网页地址的端口，此处必须使用 8088 端口。 | 必选                |
