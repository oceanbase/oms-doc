...
本文为您介绍如何在单地域多节点中使用 OMS 部署工具安装 OceanBase 迁移服务(OceanBase Migrat ion Service，OMS)。
...
下图为多节点部署的架构，Store 为日志拉取组件，JDBCWriter 和 Connector 为实时同步组件。当 OMS A 发生单点故障时，运行在 OMS A 上的 Store/JDBCWriter/Connector 组件进程会被高可用服务守护，并动态切换至 OMS B 或 OMS C 节点上。
...
|         参数          |                                                                                                                                                         描述                                                                                                                                                          |       是否必选        |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------|
| oms_meta_host       | 元信息库的 IP 地址，可以是 MySQL 数据库或 OceanBase 数据库 MySQL 租户。 <br>**注意：**<br>  OceanBase 数据库需要是 2.0 及以上版本。                                                                                                                                                                                              | 必选                |
...
| drc_user            | 用于读取 OceanBase 数据库增量日志的账号名称，需要在 sys 租户下创建该用户。详情请参见 OMS 用户指南中的《用户权限说明》。                                                                                                                                                                                                                                              | 可选                |
| drc_password        | `drc_user` 账号对应的密码。                                                                                                                                                                                                                                                                                                 | 可选                |
| cm_url              | OMS 集群管理服务的地址。例如，<http://VIP:8088>。 <br>**说明：** <br> 单地域多节点部署时，`cm_url` 需要配置为当前地域所有 CM 隶属的一个 VIP 或域名下。 <br>不建议您使用 <http://127.0.0.1:8088>，因为该地址无法扩容为多地域多节点模式。 <br>OMS 控制台的访问地址为：`部署 OMS 的宿主机的 IP 地址:8089`。例如，`http（https）://10.10.10.1:8089`。 8088 是负责调度的程序端口，8089 是网页地址的端口，此处必须使用 8088 端口。 | 必选                |
