...
* 默认值：false
    ...
    即使该值被设置为 true，如果不是 MySQL 数据库和 OceanBase 数据库 MySQL 租户之间的数据迁移任务，也会被自动设置为 false。
...
* 功能：设置 dateTime 所在的时区。

  例如，配置为 +08:00，表示 dateTime 是 +8 时区的时间。
...
* 功能：时间格式化配置。
  ...
  例如，yyyy-MM-dd HH:mm:ss，配置后 datetime 和 timestamp 会转换为根据指定格式输出时间。
