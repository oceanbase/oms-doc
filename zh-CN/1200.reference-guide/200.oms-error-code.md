...
**错误原因：** 当前任务未创建 Store，或已有 Store 进程均已退出。
**解决方案：**
...
2. 登录 OMS 社区版控制台，单击当前任务右上角 **查看组件监控** 按钮，查看组件日志，观察是否有明显报错。

3. 如果组件日志中无明显报错，单击当前任务右上角 **恢复** 按钮，尝试恢复任务。
...
企业版：请先尝试在界面上单击 **恢复** 按钮，恢复任务。如果未恢复，则在容器内执行  `supervisorctl status` 检查 OMS 社区版各组件是否均为 `RUNNING` 状态（如果不为 `RUNNING`，执行 `supervisorctl restart $组件名称` 重启该组件）。
...
企业版：登录 OMS 社区版控制台, 单击当前任务右上角的 **查看组件监控** 按钮，查看 Store 组件日志，观察是否有明显报错。
...
企业版：请提交工单寻求支持。登录 OMS 社区版控制台，单击当前任务右上角 **查看组件监控** 按钮，查看 checker 组件日志，观察是否有明显报错。
...
企业版：登录 OMS 社区版控制台，单击当前任务右上角 **查看组件监控** 按钮，查看 Connector 组件日志，观察是否有明显报错。
