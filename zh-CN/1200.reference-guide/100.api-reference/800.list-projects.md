|slug|list-projects-of-api|
|---|---|

# 分页查询任务列表

本文为您介绍 OceanBase 迁移服务（OceanBase Migration Service，OMS）社区版提供的 OpenAPI：分页查询任务列表 ListProjects。
...
| 参数   |  类型    |  描述   |
|--------|---------|------------|
|searchKey  | string |  数据迁移或数据同步任务的名称/ID。 |
| type |string  | 任务的类型，包括 `MIGRATION`（表示数据迁移任务）和 `SYNC`（表示数据同步任务）。  |
| status | string | 任务的状态。  |
| labelIds | array | 标签 ID 的集合，最多支持 5 个。  |
| sourceEndpointTypes| array | 源端数据源类型的集合。 |
|sinkEndpointTypes  | array | 目标端数据源类型的集合。 |
|  visibleSubProject| boolean | 是否展示子任务，默认不展示。  |
...
| data       | object []           | 任务列表的相关信息，具体包含的信息请参见《data 数据结构》。 |
...
* **data 数据结构**
    ...
    |  id |  string  | 任务的 ID。    |
    |  type | string   |任务的类型，包括 `MIGRATION`（表示数据迁移任务）和 `SYNC`（表示数据同步任务）。     |
    |  name |  string      |  任务的名称，不能包含空格。|
    |    ├─ id |  string  | 标签的 ID。    |
    |    ├─ name | string   |  标签的名称。   |
    |  owner | string   | 任务的 Owner。     |
    |  importance |  string  | 任务的告警等级，包括 `HIGH`、`MEDIUM`、`LOW` 和 `IGNORE`。    |
    |  status |  string  | 任务的状态。  |
    |  gmtCreate | string    | 任务的创建时间，以 UTC + 0 时区的时间格式展示。 |
    |  gmtModified | string   | 任务的更新时间，以 UTC + 0 时区的时间格式展示。     |
    |  gmtStart | string   | 任务的开启运行时间。    |
    |  gmtFinish |  string  | 任务的结束运行时间。    |
    |  destConnId |  string  |  任务的连接信息 ID。   |
    |  isMerging |  boolean  |  任务是否处于合并中。   |
    |  isModifying |  boolean  | 任务是否处于修改中。    |
    |  isSubProject |   boolean  | 是否为子任务。    |
    ...
    |    ├─ allowDestTableNotEmpty  | string  | 目标端表对象存在记录时处理策略，包括 **<UI-TERM key="oms-community.Operation.constants.Ignore">忽略</UI-TERM>** 和 **<UI-TERM key="oms-community.New.components.ParameterForm.StopProject">停止迁移</UI-TERM>**。<ul><li>设置为 true 时，表示选择忽略：当目标端表对象存在数据时，OMS 社区版将采用忽略或者覆盖的策略进行数据写入。仅目标端为 MySQL 数据库或 OceanBase 社区版时支持覆盖。  <main id="notice" type='notice'><h4>注意</h4><p>选择 <b>忽略</b>，全量校验将使用 IN 模式拉取数据，无法校验目标端存在源端没有的数据的场景，并且校验性能会有一定程度降级。</p></main></li><li>设置为 false 时，表示选停止迁移：当目标端表对象存在数据时，全量迁移会报错不允许迁移，请处理好目标端数据后再继续迁移。  <main id="notice" type='notice'><h4>注意</h4><p>如果出错后单击恢复，OMS 社区版将忽略该配置选项，继续迁移表数据，请谨慎操作。</p></main></li></ul> |
    ...
    |    ├─ startTimestamp  |  long | 增量同步起始位点。仅启用增量同步但未启用全量传输时，支持设置该参数。  |
    ...
    |    ├─ charset  |  string  | 字符编码。    |
    |    ├─ nlsLengthSemantics  | string   | nlsLengthSemantics 属性，仅针对 Oracle 和 OceanBase 数据库 Oracle 兼容模式数据源。   |
    ...
    |    ├─ stepInfo |  object  | 各步骤的运行详情，请参见 [查询任务步骤的详情](../100.api-reference/1000.describe-project-steps.md)。   |
    ...
    |    ├─ subtopics  |  string  |  任务关联 Store 的 Subtopics。 |
    |    ├─ reverseSubtopics  |  string  | 任务关联反向 Store 的 Subtopics。   |
    ...
    |    ├─ subDbs | map   | 记录不同下游消费方的表配置，用于数据同步任务复用时的配置合并。    |
    |    ├─ subConds  |  map  |  配置对象列表。  |
