...
本文为您介绍 OceanBase 迁移服务（OceanBase Migration Service，OMS）社区版提供的 OpenAPI：查询任务的详情 DescribeProject。
...
| data       | string           | 任务的相关信息，具体包含的信息请参见《data 数据结构》。 |
...
* **data 数据结构**
    ...
    |    ├─ allowDestTableNotEmpty  | string  | 目标端表对象存在记录时处理策略，包括 **<UI-TERM key="oms-community.New.components.ParameterForm.Ignore">忽略</UI-TERM>** 和 **<UI-TERM key="oms-community.New.components.ParameterForm.StopProject">停止迁移</UI-TERM>**。<ul><li>设置为 true 时，表示选择忽略：当目标端表对象存在数据时，OMS 社区版将采用忽略或者覆盖的策略进行数据写入。仅目标端为 MySQL 数据库或 OceanBase 社区版时支持覆盖。  <main id="notice" type='notice'><h4>注意</h4><p>选择 <b>忽略</b>，全量校验将使用 IN 模式拉取数据，无法校验目标端存在源端没有的数据的场景，并且校验性能会有一定程度降级。</p></main></li><li>设置为 false 时，表示选停止迁移：当目标端表对象存在数据时，全量迁移会报错不允许迁移，请处理好目标端数据后再继续迁移。  <main id="notice" type='notice'><h4>注意</h4><p>如果出错后单击恢复，OMS 社区版将忽略该配置选项，继续迁移表数据，请谨慎操作。</p></main></li></ul> |
