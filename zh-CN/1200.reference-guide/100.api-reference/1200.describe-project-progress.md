|slug|describe-project-progress-of-api|
|---|---|

# 查询任务的进度

本文为您介绍 OceanBase 迁移服务（OceanBase Migration Service，OMS）社区版提供的 OpenAPI：查询任务的进度 DescribeProjectProgress。
...
| data       | string           | 任务进度的相关信息。 |
|  ├─ projectId |  string  |  任务的 ID。  |
|  ├─ status  |  string  |  通用的任务状态枚举，包括 INIT（未启动）、RUNNING（运行中）、SUSPEND（暂停中）、FAILED（已失败）、FINISHED（已完成）、RELEASING（释放中）、RELEASED（已释放）、DELETED（已删除）、RUNNING_MODIFYING（修改中）和 RUNNING_MERGING（合并中）。  |
|  ├─ alarmLevel  |  string  | 告警级别，包括 HIGH（高保护）、MEDIUM（中保护）、LOW（低保护）和 IGNORE（无保护）。   |
|  ├─ enableIncrSync  | boolean   | 是否有正向增量同步。   |
|  ├─ currentStep  | string   |  当前任务运行所处的步骤。 |
