...
本文为您介绍 OceanBase 迁移服务（OceanBase Migration Service，OMS）社区版提供的 OpenAPI：创建任务 CreateProject。
...
|   importance | string  | 任务告警等级，包括 `IGNORE`（无保护）、`LOW`（低保护）、`MEDIUM`（中保护）和 `HIGH`（高保护）。|
|   sourceEndpointId | string | 源端数据源的 ID。您需要调用 `ListDataSource` 接口获取 ID，格式为 `e_****`。|
|   sinkEndpointId |  string | 目标端数据源的 ID。您需要调用 `ListDataSource` 接口获取 ID，格式为 `e_****`。|
|   preCheckId  |  string  | 预检查任务的 ID（可选）。如果填写 preCheckId，则当前任务的预检查步骤会使用该预检查。如果未填写 preCheckId，则会创建一个新的预检查任务。|
...
|    ├─ shardsCount | integer | 索引分片数。|
|    ├─ replicasCount | integer | 索引副本数。|
|    ├─ dateFormat  | string  | Date 格式。例如，`yyyy-MM-dd HH:mm:ss`。|
...
* **databases/databasesBlack 数据结构**
    ...
    |  ├─ filterColumns | array | 过滤列集合。   |
    |  ├─ shardColumns | array  | 分片列集合。    |
    | procedures  |array | 当前数据库中存储过程的配置。|
    |  ├─ name |  string | 存储过程的名称。    |
    |  ├─ mappedName | string  |  存储过程的映射名称。  |
    | functions  |array | 当前数据库中函数的配置。|
    |  ├─ name |  string | 函数的名称。    |
    |  ├─ mappedName | string  |  函数的映射名称。  |
