...
本文为您介绍如何多节点升级 OceanBase 迁移服务（OceanBase Migration Service，OMS）至 3.4.0 版本。
...
2. 备份数据库。
   ...
   1. 登录 IP1 和 IP2，分别暂停两台机器上的 OMS V3.3.1 旧容器。
...
8. 依次升级完两个节点后，在 **系统参数** 页面，开启 HA 功能，并设置相关参数。
   ...
   | 链路类型 |                                                                              升级前链路/组件状态                                                                              |                                                                             升级后（未开启 HA）                                                                              |                                                                                                              升级后（开启 HA）                                                                                                               |
   |------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   | 迁移链路 | 链路和组件正常运行                                                                                                                                                            | <ul><li>链路正常运行   <li> 组件 Store 和 Writer 异常     </ul>                                            | <ul><li> OMS V3.1.0 以下的版本，需要用户手动拉起 Store。 HA 会根据 `ha.config` 中配置参数 `refetchStoreIntervalMin` 的时长进行回拉。默认值为 30 分钟。   <li> HA 会自动拉起 Writer。 </ul>      |
   | 迁移链路 | 链路失败                                                                                                                                                                 | <ul><li>链路失败   <li> 组件 Store 和 Writer 异常    </ul>                                              | <ul><li> OMS V3.1.0 以下的版本，需要用户手动拉起 Store。  <li> HA 自动启动 Writer。   <li> 需要用户手动恢复链路。   </ul>                                         |
   | 同步链路 | 链路和组件正常运行                                                                                                                                                            | <ul><li> 链路正常运行  <li> 组件 Store 和 Connector 异常     </ul>                                         | <ul><li> HA 会根据 ha.config 中配置参数 `refetchStoreIntervalMin` 的时长进行回拉。默认值为 30 分钟。   <li> HA 会重新拉起 Connector。    </ul>                                                 |
   | 同步链路 | <ul><li> 链路为暂停状态   <li> Connector 为暂停状态   <li> Store 为运行状态  </ul>  | <ul><li> 链路为暂停状态   <li>Connector 为暂停状态   <li>Store 为异常状态 </ul>   | <ul><li> HA 会根据 `ha.config` 配置参数中 `refetchStoreIntervalMin` 的时长进行回拉。默认值为 30 分钟。   <li> HA 会重新拉起 Connector。   <li> 需要用户手动恢复链路。  </ul>  |
