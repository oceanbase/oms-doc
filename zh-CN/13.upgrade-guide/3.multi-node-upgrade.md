...
3. 加载下载的 OMS 社区版安装包至 Docker 容器的本地安装包仓库。
...
4. 启动 OMS 社区版 4.0.0-CE 版本的新容器。
   ...
   | IMAGE_TAG               | 镜像 TAG。您可以在宿主机上加载安装包后，运行 `docker images` 语句查看。格式为 feature_x.x.x-ce，请根据具体版本填写 x.x.x。例如，使用 OMS 社区版 4.0.0-CE 版本，则填写 feature_4.0.0-ce。                                                                                                                                      |
