...
3. 从 OMS 社区版 V4.2.3-CE 镜像拉取升级脚本 `docker_remote_deploy_410_up_423_version.sh` 至当前目录。
...
3. 检查 OMS 社区版 V4.1.0-CE 的配置文件 `config.yaml` 是否正确。

   `config.yaml` 需要和 `config_copy.yaml` 一致。如果不一致，请修改 `config.yaml` 中的配置。
