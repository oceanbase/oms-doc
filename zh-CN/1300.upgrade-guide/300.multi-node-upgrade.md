...
2. 从 OMS 社区版 V4.2.3-CE 镜像拉取升级脚本 `docker_remote_deploy_410_up_423_version.sh` 至当前目录。
...
3. 执行升级脚本，开始部署 V4.2.3-CE。
    ...
    | 配置文件路径| 必须是 OMS 社区版 V4.1.0-CE 容器生产的配置文件 `config.yaml` 的路径。 |
...
4. 根据部署工具提示完成部署，每次输入后，通过回车进行下一步。
...
6. 等待升级完成后，出现 **【结束】全部初始化步骤执行成功**，即可访问 OMS 社区版 V4.2.3-CE。
