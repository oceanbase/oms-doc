...
本文档介绍 OceanBase 迁移服务（OceanBase Migration Service，OMS）的错误信息及其解决方法。

## OMS 错误码分类说明

OMS 的错误码按照组件分类，分为 Console 错误码、预检查错误码、CM（集群管理服务）错误码和 Supervisor 错误码。更多关于 OMS 组件的信息，参考 [名词解释](../2.product-introduction/2.concepts.md)。

## OMS 错误码等级说明

OMS 的错误码等级分为 FATAL、ERROR 和 WARNING 三类，详细说明请参见下表。
...
2. 如果无法恢复原先的 Store 组件，可以尝试新建 Store。您可以登录 OMS 控制台，前往 **运维监控-组件-Store**，按照链路 Topic 信息和 Writer 当前位点时间，新建一个 Store。注意：Store 启动时间，需要稍提前于 Writer 当前位点。示例：如果 Writer 位点时间在 "2022-08-01 10:05:00"，您需要将 Store 位点提前 5~10 分钟(例如 "2022-08-01 10:00:00")，再进行新建 Store 操作。
...
2. 如果无法恢复原先的 Store 组件，可以尝试新建 Store。您可以登录 OMS 控制台, 前往 **运维监控-组件-Store**，按照链路 Topic 信息和 Writer 当前位点时间，新建一个 Store。注意：Store 启动时间，需要稍提前于 Writer 当前位点。示例：如果 Writer 位点时间在 "2022-08-01 10:05:00"，您需要将 Store 位点提前 5~10 分钟(如 "2022-08-01 10:00:00")，再进行新建 Store 操作。
...
**错误原因：** OMS 代理服务组件 Supervisor 长时间未向管控组件汇报任务执行结果。

**解决方案：**

企业版：

1. 进入 OMS 容器，执行 `supervisorctl status oms_drc_supervisor` 命令。如果状态不为 `RUNNING`，执行 `supervisorctl restart oms_drc_supervisor` 命令重启 Supervisor 组件。

2. 登录 OMS 控制台，单击当前项目右上角 **查看组件监控** 按钮，查看组件日志，观察是否有明显报错。
...
**错误原因：** OMS 代理服务组件 Supervisor 执行管控下发的命令时，发生预期外异常。

**解决方案：**

企业版：请先尝试在界面上单击 **恢复** 按钮，恢复项目。如果未恢复，则在容器内执行  `supervisorctl status` 检查 OMS 各组件是否均为 `RUNNING` 状态（如果不为 `RUNNING`，执行 `supervisorctl restart $组件名称` 重启该组件）。
...
**错误原因：** OMS 代理服务组件 Supervisor 执行管控下发的命令失败。

**解决方案：**

企业版：请先尝试恢复项目。如果未恢复，进入 OMS 容器，查看 `/home/admin/logs/supervisor/error.log` 日志，并提供给 OMS 管理员。
...
企业版：可以直接重试运维工单。或者登录 OMS 控制台, 进入 **运维监控-组件** 页面，进入具体的组件类型页面，逐个搜索失败的组件进程名称，再继续进行相应的运维操作。
...
企业版：登录 OMS 控制台, 单击当前项目右上角的 **查看组件监控** 按钮，查看 Store 组件日志，观察是否有明显报错。
...
企业版：请提交工单寻求支持。登录 OMS 控制台，单击当前项目右上角 **查看组件监控** 按钮，查看 checker 组件日志，观察是否有明显报错。
...
企业版：登录 OMS 控制台，单击当前项目右上角 **查看组件监控** 按钮，查看 Connector 组件日志，观察是否有明显报错。
...
**错误原因：** 容器内 OMS 代理服务组件 Supervisor 长时间未向数据库汇报心跳。

**解决方案：**

企业版：

1. 登录 OMS 容器，执行 `supervisorctl status` 命令。如果 OMS 组件为非 `RUNNING` 状态, 则执行 `supervisorctl restart $组件名` 重启该组件，观察是否恢复。

2. 否则继续执行 `env | grep OMS_HOST_IP` 命令，确认启动 OMS 容器时，是否正确传入 `OMS_HOST_IP` 参数。如果未传入或传入错误，请删除当前容器，正确传入参数后，重新部署。
...
**错误原因：** 未正常执行 OMS 容器初始化。

**解决方案：**

企业版：请进入 OMS 容器，再次执行 `sh /root/docker_init.sh` 命令。该操作是幂等行为，可重复执行。
...
**错误原因：** OMS 集群内机器部分资源指标超出系统阈值。其中，CPU 阈值为 `$(CPU)`，内存阈值为 `$(Mem)`，磁盘阈值为 `$(Disk)`。 各机器实际资源使用率如下：`{$Usage}`。
...
企业版：请进入 OMS 容器，执行 `sh /root/docker_init.sh` 命令，并等待命令结束。执行命令需耗时 3~5 分钟。该操作是幂等行为，可重复执行。
...
**解决方案：** OMS 内部组件转换 DDL 问题，请从 `error.log` 中找到执行报错的 DDL，根据该 DDL 再从 `ddl_msg.log` 中找到原始的 DDL，评估跳过该 DDL 的影响。如果可以跳过，则配置跳过该 DDL，并联系 OMS 管理员反馈转换 DDL 的问题。如果不能跳过，请联系 OMS 管理员反馈转换 DDL 的问题。

### CONNECTOR-SIOR01003291

**错误等级：** FATAL

**错误信息：** SQL Error: ORA-03291: Invalid truncate option - missing STORAGE keyword
...
**错误信息：** SQL Error: ORA-00907: missing right parenthesis

**错误原因：** 非法的 Oracle 语法，缺少右括号。
...
**错误信息：** SQL Error: ORA-00900: You have an error in your SQL syntax

**错误原因：** 非法的 OB-ORACLE 语法
...
**错误信息：** Exceed limit of bps

**错误原因：** DataHub 单个 Shard 限流 5M/s，OMS 同步速率过快。

**解决方案：** 扩容 DataHub 的 Shard（在 Datahub Console 上操作）。

### CONNECTOR-SIDH-{type}-001005

**错误等级：** 严重错误

**错误信息：** MalformedRecordException，Not Found Field

**错误原因：** Schema 不正确。通常是发生在 DataHub 是 Tuple 模式情况下，源端执行了 DDL 增加了字段但是在目的端（DataHub）并没有增加对应的字段。

**解决方案：**

1. 确定缺少的 Datahub Schema 字段（对比源端与目的端的 Schema）。

2. 参考 [Datahub 文档](https://help.aliyun.com/document_detail/184383.html#p-d5u-2q1-6to) 修改 Datahub Schema。

### CONNECTOR-SIDH-{type}-001006

**错误等级：** 严重错误

**错误信息：** exceed max length: 2097152

**错误原因：** DataHub 允许 String 字段最大不能超过 2M。更多信息，参考 [Datahub 文档](https://help.aliyun.com/document_detail/47441.html)。

**解决方案：** 没有办法解决，只能取消同步这张表（可以通过管控的更改同步对象取消这张表的同步）。

### CONNECTOR-SIOR00012899

**错误等级：** FATAL

**错误信息：** ORA-12899: 列 \"MOCK_DATABASE\".\"MOCK_ALLTYPE_OBORACLE_TABLE\".\"TCHAR\" 的值太大 (实际值: 13, 最大值: 3)

**错误原因：**

1. 未开启增量 DDL同步，排查是否源端做了加列的操作。

2. 已开启增量 DDL 同步，可能是 DDL 转化中未成功。

**解决方案：**

1. 未开启增量 DDL 同步，先查看源端和目标端的字符集。如果两端的字符集不相同（例如源端是 UTF8，目标端是 GBK），将目标端的列长度改成源端列的 1.5 倍以上。如果两端的字符集相同，则将目标端的列长度改成和源端一致的长度。如果仍报错，清联系 OMS 技术支持人员。

2. 已开启增量 DDL 同步，请排查 `ddl_msg.log`，搜索对应的列，找到对应的 DDL 语句，联系 OMS 技术支持人员。

### CONNECTOR-SIOO00012899

**错误等级：** FATAL

**错误信息：** (conn=1055676) ORA-12899: value too large for column \"MOCK_DATABASE\".\"MOCK_ALLTYPE_OBORACLE_TABLE\".\"TCHAR\" (actual: 19, maximum: 1)

**错误原因：**

1. 未开启增量 DDL 同步，可能是源端存在增加列的操作。
...
3. 已开启增量 DDL 同步，如果源端创建表时没有自动生成分区的语法，请排查 `ddl_msg.log`，搜索对应的表，找到对应的增加分区的 DDL 语句，联系 OMS 技术支持人员。
...
**错误信息：** 目标端表不存在，table not existed, table:MOCK_ALLTYPE_OBORACLE_TABLE,db:oms_oracle.MOCK_DATABASE

**错误原因：**

1. 未开启增量 DDL 同步，可能是源端创建了表。

2. 已开启增量 DDL 同步，可能是 DDL 转化中未成功。

**解决方案：**

1. 未开启增量 DDL 同步，在目标端将表建上。

2. 已开启增量 DDL 同步，排查 `ddl_msg.log`，搜索对应的表，找到对应的建表 DDL 语句，请联系 OMS 技术支持人员。
...
2. 如果开启增量 DDL 同步，则排查 `ddl_msg.log`，搜索对应的列，找到对应的加列语句，请联系 OMS 技术支持人员。
...
1. 可能是 OMS 与 Kafka Server 网络连通性问题导致（使用 Ping 及 telent 测试方案 1）。

2. 可能是 OMS 与 KafkaServer 网络延迟太高导致（更改 Kafka 配置项方案 2）。
...
更改配置项方式，OMS 白屏运维管控，选择对应的 Connector，单击 **更新** 按钮。
...
2. 如果 Kafka 是多副本，且 server 使用的是域名连通，可能是因为该域名在 OMS 连通比较少的机器。
...
解决方案验证：在 OMS 机器上通过 kafka 自带的 producer 脚本测试是否能成功写消息到 kafka-server。
...
跳过该错误的方法（需要是 OMS 3.4.0 版本）：增加配置项 `skipErrorCode`跳过该错误 ErrorCode 即可。
...
**错误信息：** InvokeFuncCommand failed to init datahub client with exception: {exception message}

**错误原因：** 初始化 DataHub 客户端失败。
