...
此处创建的 OceanBase 社区版用户，用于创建 OceanBase-CE 数据源时填写 **<UI-TERM key="oms-community.endpoints.MySqlForm.FormItem.DatabaseUsername">数据库用户名</UI-TERM>**。例如，填写数据库用户名为 `OMSTEST`。
...
此处创建的 DRC 用户，用于创建 OceanBase-CE 数据源时填写 **<UI-TERM key="oms-community.components.MigrationParamsConfig.MigrationParamsDetail.AdvancedConfiguration">高级选项</UI-TERM>** 中的 **<UI-TERM key="oms-community.EndpointPublic.components.OracleForm.UserName">用户名</UI-TERM>**。例如，填写 DRC 用户用户名为 `user01`。
...
如果您需要迁移无主键表，执行数据迁移任务前，请在 **源端业务租户** 下创建 `__oceanbase_inner_drc_user` 用户。
...
此处创建的 MySQL 数据库用户，用于创建 MySQL 数据源时填写 **<UI-TERM key="oms-community.endpoints.MySqlForm.FormItem.DatabaseUsername">数据库用户名</UI-TERM>**。
...
此处创建的 PostgreSQL 数据库用户，用于创建 PostgreSQL 数据源时填写 **<UI-TERM key="oms-community.endpoints.DB2Form.DatabaseUsername">数据库用户名</UI-TERM>**。
...
此处创建的 TiDB 数据库用户，用于创建 TiDB 数据源时填写 **<UI-TERM key="oms-community.pages.Endpoint.DatabaseUsername">数据库用户名</UI-TERM>**。
...
此处创建的 ElasticSearch 数据库用户，用于创建 ElasticSearch 数据源时填写 **<UI-TERM key="oms-community.pages.Endpoint.DatabaseUsername">数据库用户名</UI-TERM>**。
...
此处创建的 MongoDB 数据库用户，用于创建 MongoDB 数据源时填写 **<UI-TERM key="oms-community.pages.Endpoint.DatabaseUsername">数据库用户名</UI-TERM>**。

1. 使用 mongo 命令连接到 MongoDB 数据库实例。
...
4. 创建 role 并授权，详情请参见 [用户权限说明](../800.create-and-manage-data-sources/400.user-privileges.md)。

## StarRocks 数据源

此处创建的 StarRocks 数据库用户，用于创建 StarRocks 数据源时填写 **<UI-TERM key="oms-community.pages.Endpoint.DatabaseUsername">数据库用户名</UI-TERM>**。

1. 登录 StarRocks 数据库。

2. 在 StarRocks 数据库中创建用户，用于数据迁移。
...
此处创建的 ClickHouse 数据库用户，用于创建 ClickHouse 数据源时填写 **<UI-TERM key="oms-community.pages.Endpoint.DatabaseUsername">数据库用户名</UI-TERM>**。

1. 登录 ClickHouse 数据库用户。

2. 在 ClickHouse 数据库中创建用户，用于数据迁移。
