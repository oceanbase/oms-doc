|slug|basic-principles-of-components|
|---|---|
...
Full-Import 组件负责将静态数据和部分增量数据从任务配置的源端迁移至目标端，您迁移的每张表都会经过数据切片、数据读取、数据加工和数据应用等步骤。
...
* 目前 MySQL 数据库至 OceanBase 数据库 MySQL 兼容模式的无主键表迁移，不保证数据的最终一致性。
