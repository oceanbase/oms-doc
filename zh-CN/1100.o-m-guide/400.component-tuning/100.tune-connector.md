...
1. 查询 Incr-Sync/Full-Import 组件的组件 ID。
   ...
   3. 在 **<UI-TERM key="oms-community.pages.Migration.MigrationProject">迁移任务列表</UI-TERM>** 页面，单击目标数据迁移任务的名称，进入详情页面。

   4. 单击页面右上角的 **<UI-TERM key="oms-community.Migration.constants.ViewComponentMonitoring">查看组件监控</UI-TERM>**。

   5. 在 **<UI-TERM key="oms-community.Migration.constants.ViewComponentMonitoring">查看组件监控</UI-TERM>** 对话框，查看 **<UI-TERM>Incr-Sync 增量同步组件</UI-TERM>** 或 **<UI-TERM>Full-Import 全量导入组件</UI-TERM>** 的 **<UI-TERM key="oms-community.Detail.components.SyncMonitorDrawer.ComponentId">组件 ID</UI-TERM>**。
...
* `workerNum` 已达到上限，并且您根据数据库的实际情况判断 `metrics` 日志中 `sink` 的 `executeTime`（执行时间） 和 `commitTime`（提交时间）在正常范围内。

  1. 从任务详情页面进入 **<UI-TERM key="oms-community.Detail.components.SyncMonitorDrawer.ViewComponentMonitoring">查看组件监控</UI-TERM>** 对话框。

  2. 单击目标组件后的 **<UI-TERM key="oms-community.Operation.Machine.Update">更新</UI-TERM>**。

  3. 在 **<UI-TERM key="oms-community.Operator.Connector.useConnectorAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，鼠标悬停至 `sink` > `workerNum` 参数，单击编辑图标。
  
      如果没有该参数，请将鼠标悬停至 `sink` 参数旁边的空白处，单击添加图标。

      <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>如果您使用旁路导入方式写入数据库，可以通过修改 <code>serverParallel</code> 参数来调整旁路导入 Server 端的并发数，默认值为 8。</p>
      </main>
  ...
  6. 在 **<UI-TERM key="oms-community.Operator.Checker.useCheckerAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，单击 **<UI-TERM key="oms-community.pages.Endpoint.Confirmation">确定</UI-TERM>**。

* `workerNum` 未达到上限，`metrics` 日志中每两次之间的 GC 时间很长。

  1. 从任务详情页面进入 **<UI-TERM key="oms-community.Detail.components.SyncMonitorDrawer.ViewComponentMonitoring">查看组件监控</UI-TERM>** 对话框。

  2. 单击目标组件后的 **<UI-TERM key="oms-community.Operation.components.machineCom.Update">更新</UI-TERM>**。

  3. 在 **<UI-TERM key="oms-community.Operator.Connector.useConnectorAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，鼠标悬停至 `source` > `splitThreshold` 参数，单击编辑图标。
  ...
  6. 在 **<UI-TERM key="oms-community.Operator.JDBCWriter.useJDBCWriterAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，单击 **<UI-TERM key="oms-community.components.SyncSwitch.ConfirmStopSourceApp.Determine">确定</UI-TERM>**。

* `workerNum` 只有 `1` 或 `2`，`connector.log` 打印 `conflictKey` 或者 `deepSize` 关键字。

  1. 从任务详情页面进入 **<UI-TERM key="oms-community.Migration.constants.ViewComponentMonitoring">查看组件监控</UI-TERM>** 对话框。

  2. 单击目标组件后的 **<UI-TERM key="oms-community.Operator.Checker.DisableUpdateBtn.Update">更新</UI-TERM>**。

  3. 在 **<UI-TERM key="oms-community.Operator.Connector.useConnectorAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，鼠标悬停至 `coordinator` 参数旁边的空白处，单击添加图标。

  4. 输入需要添加的 Key Name 为 `hotKeyMerge`，单击对号。

  5. 在 **<UI-TERM key="oms-community.Operator.Checker.useCheckerAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框中，找到新增的 Key Name，其默认值为 NULL。

  6. 鼠标悬停至新增参数，单击显示的编辑图标，修改参数值为 `true`，单击确认图标。

  7. 在 **<UI-TERM key="oms-community.Operator.Connector.useConnectorAction.UpdateConfigurations">更新配置</UI-TERM>** 对话框，单击 **<UI-TERM key="oms-community.components.WorkerGradeSelect.Ok">确定</UI-TERM>**。
...
* 当前任务延迟的截图，包括任务延迟、Store 延迟和 Incr-Sync 延迟。
  
  ![latency-zh](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/oms/oms-enterprise/latency-zh.png)

  如上图所示，在一个数据迁移或数据同步任务中，存在多种延迟概念。

  | 序号 | 延迟类型   | 描述   |
  |------|----------------|-------------------|
  | 1    |    任务延迟     | <ul><li>数据迁移或数据同步任务的延迟时间以 Incr-Sync 延迟时间为准。如果存在多个 Incr-Sync，以最长的 Incr-Sync 延迟时间为准。 </li><li>数据迁移或数据同步任务的延迟时间计算和组件的延迟时间计算本身属于不同的调度存储，可能存在差异。例如，任务数量过多、调度时间过长等可能导致任务延迟时间大于组件延迟时间，但仅是展示延迟时间的差别。</li></ul>         |
